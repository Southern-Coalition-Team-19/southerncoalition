...
public class SiteUserEnUSGenApiServiceImpl extends BaseApiServiceImpl implements SiteUserEnUSGenApiService {
		...
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, "southerncoalition-enUS-SiteUser", "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				siteRequest.setJsonObject(body);
				{
					...
					eventBus.request("southerncoalition-enUS-SiteUser", json, new DeliveryOptions().addHeader("action", "postSiteUserFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("southerncoalition-enUS-SiteUser", json, new DeliveryOptions().addHeader("action", "putimportSiteUserFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
	...
	public String templateSearchPageSiteUser() {
		return Optional.ofNullable(config.getString(ComputateVertxConfigKeys.TEMPLATE_PATH)).orElse("templates") + "/enUS/SiteUserPage";
	}
	...
	public Future<SearchList<SiteUser>> searchSiteUserList(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify) {
		...
		try {
			...
			List<String> roles = Optional.ofNullable(config.getValue(ComputateVertxConfigKeys.AUTH_ROLES_REQUIRED + "_SiteUser")).map(v -> v instanceof JsonArray ? (JsonArray)v : new JsonArray(v.toString())).orElse(new JsonArray()).getList();
